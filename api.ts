...
export const moviesApi: MovieFetchers = {
  ...
  upcoming: ({ pageParam }) =>
    fetch(
      `${BASE_URL}/movie/upcoming?api_key=${API_KEY}&language=ja-JP&page=${pageParam}`
    ).then((res) => res.json()),
  nowPlaying: () =>
    fetch(
  ...
  detail: ({ queryKey }) => {
    const [_, id] = queryKey;
    return fetch(
      `${BASE_URL}/movie/${id}?api_key=${API_KEY}&append_to_response=videos&language=ja-JP`
    ).then((res) => res.json());
  },
};
...
export const tvApi: TVFetchers = {
  ...
  detail: ({ queryKey }) => {
    const [_, id] = queryKey;
    return fetch(
      `${BASE_URL}/tv/${id}?api_key=${API_KEY}&append_to_response=videos&language=ja-JP`
    ).then((res) => res.json());
  },
};
